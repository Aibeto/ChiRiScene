...
export function validateMeta(values: Record<string, unknown>): string[] {
  ...
  for (const k of keys) {
    if (!(META_FIELDS as readonly string[]).includes(k)) problems.push(`存在未知字段 ${k}`)
  }
...
export function validateFieldValue(field: WritableField, value: string | boolean): string | null {
  switch (field) {
    case 'language':
...
export function replaceTopLevelField(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const rendered =
      typeof value === 'boolean' ? String(value) : quoted ? `"${value}"` : String(value)
...
export async function writeMetaFields(
  patch: Partial<Record<WritableField, string | boolean>>
  ...
  for (const key of keys) {
    const problem = validateFieldValue(key, patch[key] as string | boolean)
  ...
  for (const key of keys) {
    const next = replaceTopLevelField(content, key, patch[key] as string | boolean)
    if (next === null) return failed<MetaSnapshot>(`未找到字段 ${key}，已放弃写入`)
    content = next
  }
