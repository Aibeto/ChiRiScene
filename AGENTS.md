...
- **整条链路目前被临时停用（2026-09-18 用户要求「暂时关闭触控升频功能」）**：`cpu_load_governor.rs` 的 `TOUCH_BOOST_SUSPENDED = true` 使 `on_touch()` 直接返回——不设升频窗口、不唤醒 Worker、不打点；`feature.yaml` 各模式的 `touch_boost_enabled` 配置保持原样不受影响，改回 `false` 即恢复。`apply_disable_touch_boost`（屏蔽内核 cpu_boost）**照常生效**，所以触摸时既无 ChiRi 升频也无内核升频。触摸检测线程照常运行（poll 开销可忽略，daemon.log 的 debug `touch-event-received` 仍会打，便于确认事件源活着）。确认长期关闭时应删配置/删链路，不要留常量。
