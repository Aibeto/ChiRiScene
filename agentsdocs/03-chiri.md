...
- 特调是完全独立调度：`src/chiri/tuned.rs` 的 `TunedGovernor`（akmode / playback / daily 共用）与 CLG 完全解耦。前台为白名单应用时由 `mod.rs` 的 scheduler_ipc 先 `cpu_governor.release()` 再 `ak_governor.init_policies()` 接管，退出前台反向释放。**全部特调模式共用这一套连续控制**（每 40ms 按组内最大占用 × headroom 直接算上限，升频立即执行），差异只在参数组；**`boost_affinity`**（参数字段，默认 true）= 是否走 boost 类亲和（cpuset 收窄 big+prime + core_ctl 保大核常在线）——游戏保响应 true，省电型特调（playback/daily）必须 false：保大核常在线与「贴负载降频」相反（空转漏电），且会把前台重线程钉到大核组被低上限压住。`is_boost_mode` 命中的特调再经 `tuned_boost_affinity()` 二次过滤。**`util_smoothing`**（参数字段，默认 1.0=不平滑）= 决策负载 EMA 系数：抖动负载下「升频立即执行」会被瞬时尖峰反复推高上限（降频又被 hold 拖住），上限均值反而高于带平滑的 CLG（2026-09-17 日志回放证实），现值 playback 0.25、akmode 0.85（2026-09-22 扫参定值，以 tuned_profiles.yaml 为准），游戏保持 1.0（响应优先）；daily 组已整体移除。**降频计时重置语义（同日修复）**：down_target 只在目标**明显回升**（> down_target + hyst）时重置计时，下探/微调只更新目标——原「档位不等即重置」在抖动下让上限卡高位降不下来（回放中平滑 util 都降不动），连续控制退化成「只升不降」。
...
- **单实例 + 延迟进出（2026-09-17 重构，原多实例已废弃）**：`src/chiri/fas_manager.rs` 的 `FasManager` 同一时刻至多一个白名单应用被接管（`instance: Option<FasInstance>`）。白名单前台 → `activate`（FasController.load_policies 快照频率 + **GovernorGuard 把各 policy 的 `scaling_governor` 切 performance**，原值快照待恢复）；失去前台不立即退出 → `request_delayed_exit` 进入 **15s 延迟期**（`FasRulesConfig.deactivate_delay_secs`，normalize 夹 1..=600，`mdocs/fas-example.yaml` 可配）：mode 保持 fas、FAS 仍持有接管（频率停在最后状态、governor=performance），期间切回白名单应用由 activate 无缝续期；到期由 1s 遥测块的 `tick()` 完成退出（reset_all_freqs + clear_game + governor 按快照恢复），并按延迟期记住的目标模式（`pending_mode_after_fas`，fas→X 的 ModeChange 被延迟时记录）重新接管——contingency/babel 走 apply 的 lab 分支，其余走 `apply_mode_takeover`。ModeChange 的 fas→非fas 分支因此**不再立即切换**（拦截在 mode_clone 更新之前，避免「文件写 default、FAS 还持着频率」的中间态）。C5 收尾 deactivate_all（DOWN/panic/进程收尾/fas_enabled=false 热重载）立即全退。事件路由：FrameUpdate/SystemLoadUpdate/温度只喂活跃实例。
...
- FAS 白名单与配置（2026-09-20 起，终态 2026-09-22）：`module/config/normal/fas.yaml` 共 5 条目、4 份配置——`com.hypergryph.endfield: endfield`、`com.levelinfinite.sgameGlobal: sgame`、`com.tencent.tmgp.sgame: sgame`（**王者荣耀国服**：国服玩家实际玩 tmgp.sgame，此前白名单只有国际版 → 国服一直 default 裸跑 4.65W；sgame.yaml 已补 per_app_profiles）、`com.tencent.tmgp.speedmobile: speedmobile`、`com.tencent.lolm: lolm`（后两款同日新增，此前 default 裸跑 prime 顶格 0.61-0.67 util、迁移 1.2-2.1 万/s、4.13/3.09W；speedmobile/lolm.yaml 抄 sgame 模板）。sgame.yaml：fps_gears [30,60,120]、target_fps [60,120]、perf_init 0.50、core_temp_threshold 45（电池温度护栏）、migration_cost_ns 400000；perf_ceil 不覆盖保持 1.0，遵守「不限制最高性能释放」硬性约束。**扩容与参数均无真机 fps 验证**（见 07）。新增 FAS 游戏 = fas.yaml 加一行 + 新建 normal/fas/<配置名>.yaml，无需改 .rs。
...
- **`util_smoothing`（2026-09-22 新增，CLG 决策负载 EMA）**：抖动负载下 max*util 每 tick 大幅摆动 → target_perf 翻摆、决策方向反复翻转、`scaling_max_freq` 高频改写（8550 QQ 实测大核 2048 tick 反转 458 次）。EMA 滤波（语义同 tuned 的 `util_smoothing`，1.0=关闭、缺省即关闭=逐位等价）后反转降约五成、写频降三成。default/reduce 配 0.5，boost 保持 1.0（响应优先）；α 0.35 可再降抖动但上限均值抬升更多。main* `max_util` 列为双语义：CLG 行写**平滑前**原始值（离线回放可自行试验系数）；tuned（akmode）行写平滑后的决策负载（2026-09-17 起语义变化）——离线回放按行来源/decision 区分，tuned 行不可再当原始 util 二次平滑。字段新增须同步：`config.rs` 字段+缺省+normalize+Default、`feature.yaml` 全部 SoC 文件（SoC 文件整份覆盖根，只改根不生效）、`mdocs/config-example.yaml`。

- **FAS 防篡改强制重写改时间基准（2026-09-24）**：`freq_force_reapply_interval` 语义由「每 N **帧**」改为「每 N **秒**」（默认 30 秒，`normalize()` 钳制最小 1——旧实现写 0 会 `% 0` panic，看门狗反复重启）。旧行为在 120fps 下等效 0.25s 一次、144fps 更密，每次对 3 个 policy 做两次写频 + 两次 `re_unmount()`（实测游戏期约 24 写 + 24 `umount2`/s 且随刷新率放大）→ 现在降至 0.2 写/s、0 `umount2`；`PolicyController::force_reapply()` 里的两次 `re_unmount()` 已删除（它只 `umount2(MNT_DETACH)`、不重开 fd，对写入路径零贡献；`verify_prev_freq` 里那次保留）。副作用：`freq_hold_frames` 的 hold 不再被高频 force 击穿。**防篡改是异常兜底语义**：ChiRi 是设备上唯一的 userspace sysfs 写频调度程序、无竞争者（口径，2026-09-26 依内核源码分析修正；频率实际决策链 = waltgov + vendor hook（OMRG/frame_boost）+ FREQ_QOS 聚合，详见 `agentsdocs/06-kernel.md`「调频路径与竞争者」，ChiRi 写 scaling_max/min 是 clamp 不是频率决策，内核 thermal QoS 钳制是合法态不算篡改），节点被改写属异常态（残留旧模块、手动调试、内核异常态）。
- 全局省电向（2026-09-23 十三轮调参定稿方向：**不封顶，控高频门槛 + 动态范围**——用户口径：除部分特调外任何在用模式保留随时到最高频的能力；控功耗靠升频门槛/速率/平滑，不靠砍上限）。8550 default 现值：up_threshold 0.88、headroom_ramp 0.08、smoothing_up 0.60、up_rate_limit_ticks 3、up_jump_threshold 0.45、slow_up_scale 0.02、down_threshold 0.62、down_rate_limit_ticks 2、down_fast_threshold 0.30、spike_decay 0.10（离线模拟 QQ −3.7~−5.9%，且新参数下出现 cap≥70% 时刻 = 既压均值又保留冲高频）；per_cluster 仅 little `up_threshold: 0.95`——A510 r1 支持 AArch32、DT 能耗模型（coeff 251）低估其能耗，EAS 会把 64 位线程吸进 little，故抬高其升频门槛而非砍上限；affinity 升核 EINVAL 已按 `pin_incapable` 标记跳过。reduce：up_threshold 0.90 + perf_ceil 0.60；boost：up_threshold 0.55。**特调封顶保留**：playback per_cluster little 0.55 / big 0.75 / prime 0.50（视频场景允许天花板）。`module/config/feature.yaml`（根）是未命中 SoC 目录机型的兜底配置与代码默认值来源；**SoC 文件是覆盖层只改根不生效**。ceil 压低后交互响应全靠 touch_boost——「不跟手」应调 boost 而非抬回 ceil。FAS（sgameGlobal 等）仍是唯一 perf_ceil 豁免。历史演进：0920「禁压 ceiling」→ 0922 日常档 ceiling 0.80+per_cluster → 0923 推翻（不封顶），引用 0920/0922 的 CLG 数字一律过期。
...
- **2026-09-20 起恢复启用**（2026-09-18 曾按用户要求临时停用）：`cpu_load_governor.rs` 的 `TOUCH_BOOST_SUSPENDED = false`，`on_touch()` 正常工作（升频窗口 + Worker 唤醒 + 打点），各模式参数由 `feature.yaml` 的 `touch_boost_enabled/ms/tiers` 决定。`apply_disable_touch_boost`（屏蔽内核 cpu_boost）**照常生效**，所以触摸时既有 ChiRi 升频、内核升频仍被屏蔽。触摸检测线程照常运行（poll 开销可忽略，daemon.log 的 debug `touch-event-received` 仍会打，便于确认事件源活着）。长期关闭应删配置/删链路，不要留常量。8550 2026-09-26 `touch_boost_tiers: 3→2`（floor 0.50→0.45 ≈1.4GHz）【临时待 A/B：滑动/打字流畅度实测定稿，不行回 3 档】；背景量化：touch=1 时 policy3 均 1.41GHz vs 无触摸 0.94GHz、触摸占 big tick 22%（QQ 45%）；触摸地板只在 Worker flush clamp 生效、不反映在 tgt 列（勿按 tgt 列全量推断地板命中）。
...
- **屏幕状态判定（2026-09-26 起，现役）**：读 Android 系统属性 `debug.tracing.screen_state`（框架在亮/灭屏时写 0/1）。判定口径（用户定）：**当前值 == 息屏判定值即息屏，其余一切值（含缺失/异常）一律亮屏**——息屏判定值 = meta.yaml `screen_off_value`（缺省 1，WebUI 配置页高级设置可改）。每次读数打 trace 日志（现值与判定）；属性缺失 warn 一条（去重）。轮询线程 `screen_detect.rs [prop] monitor_screen_state_property`（500ms）变化即更新 arc 并直推 `DaemonEvent::ScreenStateChange`；`verify_screen_state`（app_detect 主循环每轮）同口径校正。原始值经 `read_screen_prop_raw()` 由 1s 采样写入 status.csv 末列 `screen_prop`（缺失 "-"，无事件可听只能轮询）。daemon 侧 `common::SCREEN_OFF_VALUE` 原子量由 `Config::load` 同步；非法值在 parse_disk_meta 回退默认 1（不判整文件非法）。安装脚本 customize.sh 的 [screen-detect] 段（不保留配置路径）在安装期 getprop 实测为 1 时把 meta.yaml 翻转成 0——模板默认「1=息屏」与该属性原生口径「1=亮屏」相反时的自动校正。
...
- 线程层（按核心粒度放置，`pin_foreground_threads=true` 启用，每 2s 再平衡一轮、前台 PID/boost 变化立即触发）——**开销控制优先，不做逐线程每轮读 stat**：
  ...
  - **组绑定释放臂必须覆盖压力窗口外（2026-09-24/25 修复）**：Busy 绑定的回落原只在 promote 路径内、被 key_pressure 门控——压力一落，Busy 线程带着 big∪prime 收窄掩码滞留（实测 95min 末帧 1115 条 pin=1/home=-1、aff 116MB/128MB）。修复 = 释放臂放宽为 `group_bind != GroupBind::None` 且 `!key_pressure`（2s 周期块兜底轮询），`restore_group_mask` 幂等（None 早退零写入）。教训见 04。
...
- core_ctl online 读取走 `FastReader`（keep-open File + seek(0) 复用句柄，core_ctl `online_reader()` 与 affinity uclamp 快照共用）：依赖 seek(0)，**不支持 llseek 的 sysfs 节点读失败**；`state()` 首读前误报 Missing。
