...
`reduce` `default` `boost` `vector` `fas` + 特调 modes（当前只有 `akmode`）+ `down`（DOWN 停摆期间写的值）。前四档来自 `rules.yaml`（默认 `global_mode: default`，`app_modes` 为空）。`scenemode` 不在取值内——它是独立息屏轴。Yumi 设备不注册特调模式。daemon 停止后 `current_mode.chr` 是陈旧值。
...
- **key 说明实际做了什么**（用户要求）：`vector` = 全局模式切 vector 档 + 关 FAS/scenemode/特调；`contingency` / `babel` / `frozen` = 预留（2026-09-16 改名：`fastmode`→`vector`、`socremode`→`contingency`，新增两个预留；**旧 key 不兼容**，残留会被判非法重置）。界面显示名与 key 解耦，走 i18n `lab.mode.*`，**一律用英文官方译名且中英两套 locale 同值**（2026-09-17 用户要求，模式名不分语言）：`vector` 显示 Vector Breakthrough、`contingency` 显示 Contingency Contract、`babel` 显示 Babel、`frozen` 显示 Frozen；中文只留在 `lab.mode.*.detail` 描述里。三个预留条目（影响项空映射）WebUI 显示「预留」且不给启用入口
